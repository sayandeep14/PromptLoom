...
| PL-116 | **`loom fmt` deletes data.** It rebuilds files from the AST, which silently drops every `//` comment **and whole `env { … }` blocks** (a lost constraint). Fix: keep comments in the AST/lexer and format env blocks, or reuse the whitespace-only approach from Lumine's formatter. Until then `loom fmt` should not be run on files with comments or env blocks | TODO | P0 | M | — |
...
| 2026-09-21 | **Lumine formatter is whitespace-only** (PL-201): the tree-rebuilding formatter deleted comments, `env` blocks and extra parents, and `loom fmt` has the same flaw (PL-116). Body elements are no longer reordered. Quick fixes are offered only where the rewrite preserves meaning |
