...
| PL-114 | Unit tests for the remaining untested packages (`lsp`, `testrunner`, `mcp`, `blame`, `minimize`, `stale`, `starter`, `summarize`, `semantic`, `tokens`, `journal`) and CLI-level tests for commands beyond inspect/weave — **CLI layer done:** `internal/e2e/commands_test.go` runs ~40 documented commands on a real scaffolded project (exit code + output), plus the service-dependent commands' failure modes. It found two more bugs, fixed: `loom thread` scaffolds were not canonical (`loom fmt --check` failed on a brand-new file), and `loom changelog` ignored `[paths]` and saw no history after `loom init`. Left: `tui` beyond deploy/weave | DONE | P1 | L | PL-107 |
...
| PL-110 | Registry follow-ups: TLS/HSTS guidance, per-pack ownership (today one shared secret can overwrite any pack), constant-time-safe secret rotation, request logging — **Done:** publisher identities (`UPLOAD_TOKENS`) with per-pack ownership enforced inside the SQL statement (race-tested on real Postgres; upgrade from a pre-ownership database verified; legacy packs admin-only); several secrets valid at once for rotation, compared in constant time with no early exit; access log (who/method/quoted path/status/bytes/duration, never headers, queries or bodies); HSTS over HTTPS incl. trusted-proxy `X-Forwarded-Proto`; TLS, ownership and rotation docs in server/README | DONE | P1 | M | PL-103 |
...
| PL-120 | Local `.lpack` packs (`pack build` / `pack install` / `pack remove` / `pack list`) use fixed `prompts/` and `blocks/` directories and ignore `[paths]` in `loom.toml`; make them follow the configured directories consistently (found during PL-301) — **Done:** `pack build`, `install` and `remove` use `config.Dirs` (the `[paths]` of the project); the archive layout stays `prompts/` + `blocks/`, so one archive installs into any project layout. Round-trip tested for a `loom/src` project, a default one, and cross-layout | DONE | P2 | S | PL-301 |
...
| PL-403 | `loom sync` / `check-sync`: render to Claude, Copilot, Cursor and `AGENTS.md` targets from `[[targets]]`; hash-compare for drift — **Done, differently from the ticket:** `loom deploy` already rendered `[[targets]]` to Claude/Copilot/Cursor/`AGENTS.md`, so a second `sync` command would only duplicate it. Added the missing drift detector as `loom deploy --check` (writes nothing, exit 1 when a target file is missing or differs, names which; also catches a prompt that changed without a redeploy) and a `deploy` gate in `loom ci` when targets exist. Fixed: `loom deploy` printed a failure for targets that could not render and still exited 0; and the docs showed `[[targets]]` with `out` / `name` keys that do not exist (the key is `dest`), so copy-pasted config failed with a missing-dest error | DONE | P1 | M | PL-106 |
| PL-404 | `loom eval`: `.eval.toml` fixtures, LLM-judge scoring (0–100), `--record` / `--compare`, `--models`; gate in `loom ci` — **Done:** `loom eval` with `evals/<Name>.eval.toml` suites (strictly validated: typos, missing criteria, duplicate names all name file and case). A judge model grades each criterion 0-100; case score = mean; pass = score >= mark (case `min_score`, suite `threshold`, default 70, `--threshold`) AND the prompt's own contract holds. Judge replies that skip criteria, are out of range or not JSON are errors, never invented scores; the answer under test is fenced as data with an ignore-instructions rule. `--models a,b` (`model` or `provider:model`, cross-provider uses that provider's key) gives a cases x models table; `--record` writes git-friendly baselines (refused when cases errored); `--compare` fails on drops beyond `--tolerance` (5) or `--strict`; `loom ci` gets an `eval` gate when suites exist and a key is set. Built on `internal/llm` (`llm.New`, `ParseSpec`); tested with fakes plus an over-HTTP run through the real client | DONE | P1 | XL | PL-107 |
| PL-405 | Provider abstraction for LLM calls (Gemini + Anthropic + OpenAI-compatible) shared by `test`, `start`, `summarize`, `eval` — **Done:** new `internal/llm` (Gemini, Anthropic, OpenAI): provider/model/key resolution, key only in a header, scrubbed errors, model-name validation, 8 MB response cap, timeouts reported as such, status-aware errors, only answer blocks returned (Anthropic `text`, not `thinking`/`tool_use`; Gemini `thought` parts skipped). `loom test`, `summarize` and `start` now share it — `start` still had the API key in the URL and an unbounded read, and the three copies had drifted. `config.ProviderDefaults` is the single source for per-provider key variable and default model (used by config, llm, doctor and the CI test gate) | DONE | P1 | M | PL-107 |
...
| ID | Ticket | Status | Pri | Size | Depends |
|---|---|---|---|---|---|
| PL-501 | Design doc: agent runtime scope (streaming, tool use, multi-turn, safety model, how it interacts with LoomLocker and `permission` in `.loom.config`) — **Done:** `docs/AGENT_RUNTIME.md`: scope (streaming, multi-turn and contract checks in v1; **no tool use, no model-driven actions**), a four-part safety model (the model can act on nothing; what leaves the machine and its guards; untrusted replies; secrets and LoomLocker), the interface and exit codes, and the six conditions any future tool use must meet (per-tool opt-in via `capabilities`, path permissions, confirmation by default, audit, LoomLocker awareness) | DONE | P1 | M | PL-405 |
| PL-502 | `loom run <Name>`: execute a resolved prompt against a model with streaming output — **Done:** `loom run <Name>`: renders exactly like `weave` (shared `renderSingle`), streams the answer (Gemini/Anthropic/OpenAI SSE in `internal/llm`, idle timeout, partial text kept on Ctrl-C, best-effort usage), `--chat` with local history / `/reset` / `/show` / turn limit, `--check` (contract), `--json`, `--out` transcript (mode 0600), `--dry-run` (sends nothing, needs no key). Safety: `permission.read` / `permission.write` from `.loom.config` are now **enforced** (checked before anything is read or sent; symlinks resolved; git/bundle sources need unrestricted read), terminal control sequences in replies are stripped (stateful, so sequences split across chunks are caught; piped output untouched), a LoomLocker placeholder in place of the API key is recognised with a fix. Found and fixed on the way: **`loom init` wrote `.loom.config` where nothing looks for it** (`loom/`), so `loom execute`, loomlocker and the client libraries could not find it and its `loom/.loom.secret` path resolved to `loom/loom/...`; `init` now writes it to the project root and moves a legacy copy | DONE | P1 | L | PL-501 |
| PL-503 | Refine / decide loop (`loom optimize`, `loom score`, `--refine`) — **Done:** `loom score` (a prompt's eval mean as one number, `--fail-under` for a release gate), `loom optimize` (scores a prompt with its eval suite, proposes a fix, shows the diff, `--yes` to apply/iterate/auto-revert-on-regression/stop-on-plateau), and `loom eval --refine`/`--yes` (one optimize iteration per failing prompt). Built on two new safe primitives: `format.ReplaceFields` (rewrites only a node's field content; provably cannot change structure, since it only ever copies parents/use/vars/variants/env/contract/capabilities from the ORIGINAL node — 8 mutation-tested guards) and `internal/optimize` (feedback extraction, a refiner call that is rejected — not silently truncated — if it changes anything but field text, a bounded regression-guarded improve loop). Every write goes through `permission.write` in `.loom.config` and needs `--yes`; without it, always a preview, nothing written. `docs/AGENT_RUNTIME.md` gained an explicit "Exception: loom optimize" section explaining why this narrow, diff-shown, structurally-locked write does not contradict rule 1 (the model can act on nothing) | DONE | P2 | L | PL-502, PL-404 |
...
| PL-605 | Add JSON secret paths (`.json:{key}`), a `loomlocker` README, publish libraries (PyPI, Maven Central) — **Done, with one step that needs you:** (1) JSON secret paths `config.json:{db.password}` / `servers.0.password`, edited as text at the value's byte span like YAML (byte-exact round trip incl. escapes, unicode keys, BOM, CRLF, no final newline; locked file stays valid JSON of the same shape; strings only, strict JSON, duplicate keys and bare `config.json` refused with clear errors, nothing written on any failure). (2) `loomlocker/README.md` (the client libraries all linked to a README that did not exist) and DESIGN/README updates. (3) Library publishing prepared and verified: found and fixed that **bloompy could not be built at all** (`build-backend` named a module that does not exist), now builds, passes `twine check` and installs/imports in a clean venv; loomj has Central-ready metadata and a `release` profile (sources + javadoc jars, GPG, Central Portal plugin) that builds; `release-libs.yml` publishes per-library tags after checking tag = version and running tests, PyPI via trusted publishing (no token); CI now builds both packages on every PR; `libs/PUBLISHING.md` lists the one-time account steps. **Not done (needs your accounts):** the actual uploads. Also decide the Maven `groupId`: `dev.promptloom` needs proof you own promptloom.dev, `io.github.sayandeep14` needs none | DONE (uploads pending accounts) | P2 | L | PL-603 |
